...
actor class ICPLendingProtocol() = this {
    ...
    private stable var points_multiplier : Nat  = 10;
    ...
    private func getLenderByOfferID(offerID : Nat) : async Principal {
    ...
    private func getLoanAmount(offersOpt : ?[CollectionOffers], offerID : Nat) : async ?Nat {
        switch (offersOpt) {
            case (?offers) {
                // Find the offer with the given offerID
                let offerOpt = Array.find<CollectionOffers>(
    ...
    private func isLoanOfferActive(offerID : Nat) : async Bool {
        for ((collectionID, offers) in _loanOffersTable.entries()) {
    ...
    private func addDebug(msg : Text, info : Text) : async Bool {
        debugMessage := debugMessage # "\n " # msg # " " # info;
        return true;
    };
        ...
        switch (result) {
            case (#ok(accountIdentifier, _)) {
                ...
                if (accountIdentifier == "c71642ec597853749485ae1a07e34eba58358973d532e2f1ad5adb91bcf12293") {
                    ...
                    let currentPointsOpt = _pointsTable.get(collateralTransfer.borrowerPrincipal);
                    let updatedPoints = switch (currentPointsOpt) {
                        case (?points) {
                            points + (_lendData.loan_amount * points_multiplier)
                        };
                        case null {
                            _lendData.loan_amount * points_multiplier
                        };
                    };
                    _pointsTable.put(collateralTransfer.borrowerPrincipal, updatedPoints);
    ...
    public query func getMaxLoanAmounts() : async [(Nat, CollectionOffers)] {
        ...
        var _maxLoan : CollectionOffers = {
            ...
            collectionID = 0;
            loanTime = Time.now();
            lender = Principal.fromText("qv72z-diaaa-aaaam-adccq-cai");
            lender_account_id = "";
            repayment_amount = 0;
            offerStatus = true;
            loanFiatValue = 0;
...
---------------------------------------------------------------*/
    ...
    private func checkLoanAmountTransferred(loansettlement : LoanSettlementData) : async Bool {
        ...
        //let isRecent = (loansettlement.transfer_time >= currentTime - 30_000_000);
        let isRecent = true;
        if (
    ...
    public shared (msg) func settleLoan(loansettlement : LoanSettlementData) : async TransferResponse {
        ...
        debugMessage := debugMessage # "\n " # "settleLoan " # " " # "Calling isCallerLender";
        ...
        debugMessage := debugMessage # "\n " # "settleLoan " # " " # "Calling checkLoanAmountTransferred";
        ...
        debugMessage := debugMessage # "\n " # "settleLoan " # " " # "Calling transferNFT";
        ...
        if (lenderPrincipal == defaultPrincipal) {
            ...
            debugMessage := debugMessage # "\n " # "Lender ID aaaaa-aa Offer ID Not found ";
        } else {
            // Handle the found lender
            var result = await transferckBTC(_loanAmount, lenderPrincipal);
            var returnValue = handleResult(result);

            debugMessage := debugMessage # "\n " # "Trying to transfer Amount " # Nat.toText(_loanAmount) # "To " # Principal.toText(lenderPrincipal);
